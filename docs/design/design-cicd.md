...
| `sync.yml`（新增）     | Cloud Scheduler 觸發 Sync 流程，執行後自動導出 `tasks.json`，並一併 commit-back，最後利用 `workflow_call` 觸發 Notify 流程。 |
| `notify.yml`（新增）   | 藉由 `sync.yml` 連動觸發，推播今日任務至 Discord / Email。                                                                   |
| `deploy.yml`（新增）   | 當 GitHub Pages 的 index.html 有異動時觸發，進行站點靜態部署。                                                               |
...
所有排程由 **GCP Cloud Scheduler** 統一管理，以 **台灣時間 (UTC+8)** 設定：

| Workflow   | Cloud Scheduler (TWN) | 對應 UTC | 說明                                       |
| ---------- | --------------------- | -------- | ------------------------------------------ |
| `sync.yml` | 00:00:05              | 16:00:05 | 每日活動同步、處理頁面資料導出，並觸發推播 |

> **不再使用 GitHub Actions Cron**：GitHub Cron 延遲不穩定（高峰時段可達 30–60 分鐘），已全面改由 GCP Cloud Scheduler 觸發 `workflow_dispatch`，確保準時執行。

### 2.7 外部排程觸發（GCP Cloud Scheduler）

GitHub Actions 的 Cron 排程延遲不穩定（高峰時段可達 30–60 分鐘），因此 **所有排程均由 GCP Cloud Scheduler 觸發**，透過 GitHub REST API 發送 `workflow_dispatch` 事件，不再使用 GitHub Actions 的 `schedule` cron。
...
flowchart LR
    ...
    GH_API -->|"觸發"| SYNC["sync.yml<br/>(workflow_dispatch)"]
```
...
| Cloud Scheduler 觸發     | `sync.yml`    | 定時同步活動資料（透過 workflow_dispatch） |
| `workflow_call`          | `notify.yml`  | 被 `sync.yml` 連動觸發                     |
| `workflow_dispatch` 手動 | 全部          | 手動觸發（除錯、臨時操作）                 |
...
**職責**：由 GCP Cloud Scheduler 觸發或手動觸發，執行 `scheduler sync`，同步完成後若 DB 有變更則自動 commit-back。
...
> 不再使用 `schedule` cron。由 Workflow `workflow_call` 與 `workflow_dispatch` 觸發。
...
%%{init: {'theme':'dark'}}%%
flowchart TD
    TRIGGER["Cloud Scheduler 觸發 / 手動 dispatch"] --> CHECKOUT["Checkout repo"]
    ...
    SYNC --> CHECK_DIFF{"git diff --quiet<br/>data & gh-pages?"}
    CHECK_DIFF -->|"有變更"| COMMIT["git add data/ gh-pages/<br/>git commit<br/>git push"]
    COMMIT --> FIRE_NOTIFY["以 workflow_call<br/>觸發 notify"]
    CHECK_DIFF -->|"無變更"| DONE["結束"]
    FIRE_NOTIFY --> DONE
```
...
**Step 4: Commit-back & Workflow Dispatch**
```yaml
- name: Commit and push DB changes
  run: |
    git diff --quiet data/line_tasks.db data/tasks.json && exit 0
    git config user.name "github-actions[bot]"
    git config user.email "github-actions[bot]@users.noreply.github.com"
    git add data/line_tasks.db data/tasks.json
    git commit -m "chore(data): auto-sync line_tasks.db and tasks.json"
    git push
```
...
| #   | 場景                            | 預期行為                                               |
| --- | ------------------------------- | ------------------------------------------------------ |
| S1  | Cloud Scheduler 觸發，DB 有變更 | Sync 執行成功；DB 自動 commit 並 push                  |
| S2  | Cloud Scheduler 觸發，DB 無變更 | Sync 執行成功；不產生 commit                           |
| S3  | `workflow_dispatch` 手動觸發    | 手動觸發完整 Sync + commit-back 流程                   |
...
**職責**：由 GCP Cloud Scheduler 觸發或手動觸發，執行 `scheduler notify`，推播明日任務清單至 Discord 與 Email。
...
> 此為被 `sync.yml` 連動的 workflow_call 階段。手動觸發時支援 `date` 參數（格式：`YYYY-MM-DD`），方便指定特定日期的推播（除錯用途）。
...
%%{init: {'theme':'dark'}}%%
flowchart TD
    TRIGGER["Cloud Scheduler 觸發 / 手動 dispatch"] --> CHECKOUT["Checkout repo<br/>(含 data/line_tasks.db)"]
    CHECKOUT --> SETUP_GO["Setup Go 1.22"]
    SETUP_GO --> CACHE["Restore Go Module Cache"]
...
| #   | 場景                                | 預期行為                                   |
| --- | ----------------------------------- | ------------------------------------------ |
| N1  | 被 Workflow Call 觸發               | Notify 執行成功；Discord 與 Email 收到推播 |
| N2  | `workflow_dispatch` 手動觸發無 date | 預設推播當日任務                           |
| N3  | `workflow_dispatch` 指定 date       | 推播指定日期狀態                           |
| N4  | Discord 發送失敗                    | Workflow 標記為 failed；Email 仍然嘗試發送 |
| N5  | Email 發送失敗                      | Workflow 標記為 failed                     |

---

### 3.4 部署站台排程 — `deploy.yml`

**職責**：將 GitHub Pages 的前端 UI `gh-pages/index.html` 建構與部署至 GitHub Pages 網域。
...
> 使用原生的 Github Actions deploy steps (`actions/upload-pages-artifact`, `actions/deploy-pages`) 處理 Pages 生命週期，並限定 `index.html` 異動才發動，與資料爬梳 (Sync) 隔離，大幅降低平日運行成本。
...
| sync/notify 不監聽 push | `sync.yml` 與 `notify.yml` 僅有 `workflow_dispatch` 觸發，不受 push 事件影響 |
...
| Sync 排程              | Cloud Scheduler 觸發或手動 `workflow_dispatch` | Actions 頁面顯示 Sync 成功；DB commit 出現於 Git 歷史                        |
