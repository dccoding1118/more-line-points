...
| `sync.yml`（新增）     | Cloud Scheduler 觸發 Sync 流程，執行後自動 commit-back `data/line_tasks.db`。 |
| `notify.yml`（新增）   | Cloud Scheduler 觸發 Notify 流程，推播明日任務至 Discord / Email。            |
...
| ------------ | --------------------- | -------- | ------------ |
| `sync.yml`   | 00:00:05              | 16:00:05 | 每日活動同步 |
| `notify.yml` | 00:02:00              | 16:02:00 | 每日任務推播 |
...
| Cloud Scheduler 觸發     | `notify.yml`  | 定時推播明日任務（透過 workflow_dispatch） |
...
> 不再使用 `schedule` cron。所有排程由 GCP Cloud Scheduler 透過 GitHub REST API 觸發 `workflow_dispatch`。
...
| #   | 場景                                | 預期行為                                   |
| --- | ----------------------------------- | ------------------------------------------ |
| N1  | Cloud Scheduler 觸發                | Notify 執行成功；Discord 與 Email 收到推播 |
| N2  | `workflow_dispatch` 手動觸發無 date | 預設推播明日任務                           |
